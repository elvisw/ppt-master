...
5. Run the matching command (see the [calculator documentation](../../scripts/docs/svg-pipeline.md#svg_position_calculatorpy) and recipes for decomposable, partial, formula, and manual modes):
   ...
   uvx ppt-master svg-position-calc calc bar --data "L1:V1,L2:V2" --area "x_min,y_min,x_max,y_max" --gap-width <payload gap_width> --value-range=0,axis_max   # Native-ready: equal category slots; default gap_width=150
   uvx ppt-master svg-position-calc calc line --data "x1:y1,x2:y2" --area "x_min,y_min,x_max,y_max" --y-range=0,max
