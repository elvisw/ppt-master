...
```bash
uvx ppt-master svg-quality-check "<workspace_root>/templates" --template-mode --canonical-authoring
```
...
```bash
uvx ppt-master apply-template <project_path> --root <workspace_root> [--root <workspace_root> ...]
```
