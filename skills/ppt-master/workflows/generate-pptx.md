...
uvx ppt-master project import-sources <project_path> <source_files_or_dirs...>   # skip when content is only in conversation
```
...
|---|---|---|
| Prepared derivative | `image-generator.md` §4.4 only for registered layers | after its canonical source is terminal: `uvx ppt-master image-treat ...` for blur, desaturation/grayscale, duotone, brightness, or contrast, or the §4.4 preparation path |
...
6. `uvx ppt-master analyze-images <project_path>/images` so the CSV reflects every placeable image.
