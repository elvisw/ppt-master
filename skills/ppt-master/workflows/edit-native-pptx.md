...
```bash
uvx ppt-master pptx-to-svg "<source.pptx>" -o "projects/<slug>_<YYYYMMDD>" --inheritance-mode both --roundtrip
```
...
```bash
uvx ppt-master svg-authoring-view "projects/<slug>_<YYYYMMDD>/authoring-svg-flat" --adopt-object slide_05.svg:<element-id> --into chapter_market.svg
```
...
```bash
uvx ppt-master svg-authoring-view "projects/<slug>_<YYYYMMDD>/authoring-svg-flat" --refresh-summary
uvx ppt-master svg-quality-check "projects/<slug>_<YYYYMMDD>" --roundtrip --json
```
...
```bash
uvx ppt-master svg-to-pptx "projects/<slug>_<YYYYMMDD>" --roundtrip
```
