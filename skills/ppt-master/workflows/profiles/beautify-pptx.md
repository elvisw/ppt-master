...
```bash
uvx ppt-master project init <project_name> [--format <format>]                 # Default
uvx ppt-master project init <project_name> [--format <format>] --quick-generate # Quick — run exactly one init
...
uvx ppt-master extract-svg-assets <project_path>/analysis/source_svg_import/svg-flat --icons-dir <project_path>/analysis/source_svg_import/icons --icon-namespace imported --inplace --id-prefix source_flat --min-decoration-bytes 3000 --clean-stale
```
...
```bash
uvx ppt-master beautify-inventory <project_path>/analysis/<stem>.slide_library.json --images <project_path>/images/image_manifest.json -o <project_path>/analysis/beautify_inventory.json
```
