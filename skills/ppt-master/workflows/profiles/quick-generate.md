...
| PDF / DOCX / Office / XLSX / XLSM / PPTX / EPUB / HTML / LaTeX / RST / web URL | `uvx ppt-master source-to-md <file_or_URL_or_dir> [...]` (`-t <type>` only when detection is ambiguous; `-o` only for a required output path) |
...
```bash
uvx ppt-master project init <project_name> --quick-generate
...
```bash
uvx ppt-master svg-to-pptx <project_path> --quick-generate --with-notes   # Speaker Notes enabled
uvx ppt-master svg-to-pptx <project_path> --quick-generate --no-notes     # Speaker Notes disabled
```
