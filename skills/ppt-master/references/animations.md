...
```bash
uvx ppt-master animation-config list-groups <project>   # real anchors after regrouping
uvx ppt-master animation-config scaffold <project>      # neutral editable scaffold (effect: none, {} placeholders)
uvx ppt-master animation-config validate <project>      # before export
uvx ppt-master svg-to-pptx <project>                    # reads <project>/animations.json automatically
```
...
```bash
uvx ppt-master sound-sync list --query <term>                       # optional exact filtering after the vocabulary is in context
uvx ppt-master sound-sync <project_path> <namespace>/<sound_id> [...]  # materialize only the chosen ids
```
...
uvx ppt-master svg-to-pptx <project> --auto-advance 5            # kiosk playback
